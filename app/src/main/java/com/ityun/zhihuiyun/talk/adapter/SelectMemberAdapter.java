...
public class SelectMemberAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
        if (position == 0) {
            ...
            final ItemUserHolder itemUserHolder = (ItemUserHolder) holder;
            ...
            if (mlist.get(position - 1).getMemberState() == 1) {
                itemUserHolder.item_user_check.setChecked(true);
            } else {
                itemUserHolder.item_user_check.setChecked(false);
            }

            itemUserHolder.item_select_ll.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    itemUserHolder.item_user_check.setChecked(!itemUserHolder.item_user_check.isChecked());
                    if (onItemOnClick != null) {
                        onItemOnClick.onClick(position - 1, itemUserHolder.item_user_check.isChecked());
                    }
                }
            });
        }
